function CutPicture(OrgFolder, destFolder, PlatesVec, circleVec, startInd,board)
%% CutPicture(OrgFolder, destFolder, PlatesVec, circleVec, board)
% ----------------------------------------------------------------------
...
% ---------
% updates: 2/7/2014
% Omri Hen, added board from prepare pictures so that the subfolder will be
% named correctly.
...
for k=1:NumOfPlates
    dirName = [destFolder,'\', num2str(board),'_', num2str(PlatesVec(k))];
...
for k=1:NumOfFiles
    ...
    for j=1:NumOfPlates%2:size(X,1)
        ...
        dirName = [destFolder ,'\',num2str(board),'_',num2str(PlatesVec(j))];
        picDir = fullfile(dirName, 'Pictures');
